...
read_cache_file(plugin_data *data)
{
    ...
    if  (G_LIKELY(get_astro_data_for_day(data->astrodata, 0)))
       ...
       data->astro_update->attempt = 0;
       data->astro_update->next = time_calc(now_tm, 0, 0, 0, 0, 0, 30);
    }
